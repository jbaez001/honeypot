...
ALL_PLATFORMS := linux/amd64 linux/arm linux/arm64 linux/ppc64le linux/s390x darwin/amd64 darwin/arm64
...
BUILD_IMAGE ?= golang:1.22.6-bookworm
...
all-container all-containers: # @HELP builds containers for all platforms
all-container all-containers: $(addprefix container-, $(subst /,_, $(ALL_PLATFORMS)))
...
clean: container-clean bin-clean node-clean
